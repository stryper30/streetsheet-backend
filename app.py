...
def fill_timesheet():
    try:
        ...
        for i, leave in enumerate(leave_data):
            ...
            col_map = {'PTO': 2, 'Comp': 3, 'Sick': 4, 'Bereavement': 5}
            if l_type in col_map:
                ws.cell(row=leave_row, column=col_map[l_type]).value = float(l_hours)
